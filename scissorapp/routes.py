...
from flask import Blueprint, render_template, url_for, request, redirect, flash, abort, send_file, current_app as app, send_from_directory
...
@short.route('/create_link', methods=['POST'])
...
def create_link():
    ...
    # Add a default schema if the URL doesn't have one {turns www.google.com to https://www.google.com}
    ...
    if custom_url:
        ...
        if existing_link:
            return render_template('link_success.html', custom_url_exists=True, new_link=existing_link.custom_url, original_url=existing_link.original_url, qr_code_url=existing_link.qr_code_url, visits=existing_link.visits)
        # if it isn't found in the database, add it to the Link object
            ...
            qr_code_url = generate_qr_code(link.original_url, custom_url)
            link.qr_code_url = qr_code_url  # Generate QR code for the short URL and save it with the custom url
            link.save()
            return render_template('link_success.html', new_link=link.custom_url, original_url=link.original_url, qr_code_url=qr_code_url)
    ...
    if short_link:
        return render_template('link_success.html', new_link=short_link.short_url, original_url=short_link.original_url, qr_code_url=short_link.qr_code_url, visits=short_link.visits)
    ...
    # if it isn't, create a new Link object, save it, and return the new short url
        ...
        qr_code_url = generate_qr_code(link.original_url, link.short_url)
        link.qr_code_url = qr_code_url # Generate QR code for the short URL and save it with the short url
        link.save()
        return render_template('link_success.html', new_link=link.short_url, original_url=link.original_url, qr_code_url=qr_code_url)
...
@short.route('/qr_code/<filename>')
def serve_qr_code(filename):
    # Specify the directory where QR code images are stored
    qr_code_folder = os.path.join(app.root_path, 'static', 'image')

    # Return the requested QR code image file
...
def download_qr_code(qr_code_url):
    ...
    qr_code_path = os.path.join(app.root_path, 'static', 'image', qr_code_url)
